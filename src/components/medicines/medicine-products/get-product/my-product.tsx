...
export default function MyProduct() {
  ...
  const myProducts = products.filter((product) => product.owner === address);

  const handleCardClick = (product: Product) => {
    if (!product.isSold) setSelectedProduct(product);

    setSoldOutMessage("This product is out of stock, please choose another product.");
  };
  ...
  return (
    <motion.section
      className="max-w-[90%] w-full my-20 mx-auto"
      variants={containerVariants}
      initial="hidden"
      animate="visible"
    >
      <div className="flex justify-center items-center mb-6">
        <h2 className="text-2xl md:text-4xl font-bold">Your Products</h2>
      </div>
